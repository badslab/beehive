...
def md(**kwargs):
    ...
    if 'pubmed' in kwargs:
        ...
        termite.h5ad.set1(globals.adata, 'author', pmd['author'])
        termite.h5ad.set1(globals.adata, 'title', pmd['title'])
        termite.h5ad.set1(globals.adata, 'year', pmd['year'])
        termite.h5ad.set1(globals.adata, 'abstract', pmd['abstract'])
        termite.h5ad.set1(globals.adata, 'doi', pmd['doi'])

    for k, v in kwargs.items():
        termite.h5ad.set1(globals.adata, k, v)
    
    for k, v in globals.adata.uns['termite']['metadata'].items():
        print(k + ": " + "\n".join(
...
def get_layerdata():
    ...
    ldata = globals.adata.uns['termite']['layers']
    d = []
    ...
    check_layer('X', globals.adata.X)
    for layer in globals.adata.layers.keys():
        check_layer(layer, globals.adata.layers[layer])

    df = pd.DataFrame(d)
    return df


def obsm(name: Optional[str] = None,
         load: bool = True):
    adata = globals.adata
...
def layers(name=None, ltype=None, load=None) -> Optional[pd.DataFrame]:
...
def save(filename: Optional[str] = None):
    ...
    globals.adata.write_h5ad(h5adfile)
